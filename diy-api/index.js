...
app.get("/random", (req, res) => {
  const joke = jokes[randomNumber(0,jokes.length)];
  res.send(joke);
})
...
app.post('/jokes', (req,res) => {
  ...
  jokes.push(newJoke);
  res.json(jokes.find((item) => item.id == newJoke.id));
})


//5. PUT a joke
app.put('/jokes/:id', (req,res) => {
  const updateJoke = jokes.find((item) => item.id == req.params.id);
  
  updateJoke.jokeText = req.body.text;
  updateJoke.jokeType = req.body.type; 
